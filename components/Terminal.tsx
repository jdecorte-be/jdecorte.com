...
const Terminal = () => {
	...
	const executeCommand = useCallback((cmd: string) => {
		...
		if (commands[trimmedCmd]) {
			...
			const suggestions = [...Object.keys(commands), "clear"].filter(
				(c) =>
	...
	const typeWelcomeMessage = useCallback(async () => {
		...
		for (let i = 0; i <= welcomeMessage.length; i++) {
			...
			setHistory((prev) => {
				...
				if (
					newHistory.length > 0 &&
					newHistory[newHistory.length - 1].cmd === "welcome"
				) {
					newHistory[newHistory.length - 1].output = [
						welcomeMessage.slice(0, i),
					];
				}
				return newHistory;
			});
		...
		setHistory((prev) => [
			...prev,
			{ cmd: "help", output: commands.help.execute() },
		]);
					...
					if (matches.length === 1) {
						...
						setHistory((prev) => [
							...prev,
							{
								cmd: "system",
								output: [
									`${matches.length} possible completions:`,
									...matches.sort(),
								],
							},
						]);
	...
	const renderHistoryItem = (item: Command, idx: number) => {
		if (item.cmd === "banner") {
			return (
				<pre
					key={idx}
					className="text-cyan-400 text-sm mb-4 whitespace-pre-wrap leading-none"
				>
					{item.output.join("\n")}
				</pre>
			);
		...
		>
			<div className="justify-center">
						...
						<form onSubmit={handleSubmit} className="flex items-center">
							{isWelcomeDone ? (
								...
								<div className="text-blue-400 font-bold">
									Initializing terminal, please wait...
								</div>
							)}
						</form>
					</div>
				...
				<div className="text-center text-xs text-gray-500 mt-4">
					<p>
						Tip: Use <kbd>↑</kbd> and <kbd>↓</kbd> to navigate command history,{" "}
						<kbd>Tab</kbd> for autocomplete.
					</p>
				</div>
			</div>
		</div>
