---
title: 'Minishell: Building a Mini-Bash Shell from Scratch in C'
date: '2022-04-18'
tags: ['C', 'Unix', 'Systems Programming', 'Shell', 'Process Management']
draft: false
images: ['/static/images/portfolio/minishell/main3-poster.avif']
summary: "A comprehensive guide to building a functional shell interpreter from scratch, covering pipes, process management, signal handling, redirections, and command execution."
---
...
![Process Hierarchy](/static/images/minishell/process-hierarchy.avif)
...
![Fork Example Output](/static/images/minishell/fork-example.avif)
...
![Memory Not Shared](/static/images/minishell/memory-not-shared.avif)
...
![Wait Zombie Process](/static/images/minishell/wait-zombie.avif)

Conversely, if a parent exits before waiting, children become orphans and are adopted by `init`:

![Wait Orphan Process](/static/images/minishell/wait-orphan.avif)
...
![Wait Analysis Output](/static/images/minishell/wait-analysis.avif)

And here's a comparison with different exit codes:

![Wait Exit Codes Comparison](/static/images/minishell/wait-exit-codes.avif)
...
![Pipe Diagram](/static/images/minishell/pipe-diagram.avif)
...
![Pipe Example Output](/static/images/minishell/pipe-example-output.avif)
...
![Pipe Blocked Output](/static/images/minishell/pipe-blocked-output.avif)
...
![Shell Pipe Diagram](/static/images/minishell/shell-pipe-diagram.avif)
...
A **signal** is an asynchronous notification sent to a process. Common signals include `SIGINT` (2, Interrupt via Ctrl-C), `SIGQUIT` (3, Quit via Ctrl-\), `SIGTERM` (15, Termination request), `SIGKILL` (9, Force kill that cannot be caught!), and `SIGSTOP` (19, Stop process that cannot be caught!).

Here's a visual representation of how signals work at the system level:

![Signal Diagram](/static/images/minishell/signal-diagram.avif)
...
![Signal Pending](/static/images/minishell/signal-pending.avif)
...
![Signal Handler Output](/static/images/minishell/signal-handler-output.avif)
...
The shell needs to behave as follows: **Ctrl-C (SIGINT)** displays a new prompt line (interrupt current command), **Ctrl-D** exits the shell (EOF), and **Ctrl-\ (SIGQUIT)** does nothing (ignore).
...
![Signal Blocking Output](/static/images/minishell/signal-blocking-output.avif)

And what happens when a blocked signal is unblocked (notice how the pending signal is delivered immediately):

![Signal Unblock Output](/static/images/minishell/signal-unblock-output.avif)

## Redirections

Shells support redirecting input and output:

The shell supports four redirection operators: `>` (redirect output, overwrite), `>>` (redirect output, append), `<` (redirect input), and `<<` (here-document, read until delimiter).
...
The shell handles two special variables: `$?` (exit status of last command) and `$$` (current shell's PID).
...
Parse the tokens into this structure, handling quotes (single and double), variable expansion, whitespace, and special characters.
...
With all the forking and string manipulation, leaks are easy. Use Valgrind religiously, free everything in both parent and child paths, and be especially careful with readline's returned strings.

### 2. File Descriptor Leaks

Unclosed file descriptors accumulate and cause mysterious bugs. Track all opens/pipes with a list, close in both parent and child, and use `lsof -p <pid>` to debug.

### 3. Zombie Processes

Children not properly waited for become zombies. Always `wait()` or `waitpid()` for children, and use `WNOHANG` if you need non-blocking checks.

### 4. Race Conditions

With multiple processes and signals, block signals during critical sections, use proper signal-safe functions, and be careful with shared resources.
...
Several community testers exist: [minishell_tester](https://github.com/LucasKuhn/minishell_tester) and [42_minishell_tester](https://github.com/zstenger93/42_minishell_tester).
...
Test edge cases like empty input, commands with only whitespace, unclosed quotes, invalid redirections, permission errors, non-existent commands, and signal delivery during system calls.
...
The project forces you to understand how processes actually work, the beauty and complexity of Unix pipes, why signal handling is so tricky, and what shells do when you type commands.

## Conclusion

Minishell is more than just a project—it's a deep dive into Unix fundamentals. By building a shell from scratch, you gain intimate knowledge of process creation and management, inter-process communication, signal handling, file descriptors and I/O redirection, and command parsing and execution.
...
Key resources include the [GNU Bash Manual](https://www.gnu.org/software/bash/manual/), man pages (`man bash`, `man fork`, `man pipe`, `man signal`), [The Linux Programming Interface](https://man7.org/tlpi/) by Michael Kerrisk, [Advanced Programming in the UNIX Environment](https://www.apuebook.com/) by Stevens & Rago, and the [Minishell Tutorial Series](https://www.codequoi.com/).
